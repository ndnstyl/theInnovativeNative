...
3. **Font availability on n8n server.** The spec calls for Anton font. Haven learned (WF-006) that the n8n Docker container only has DejaVuSans. Anton would need to be installed in the container or baked into a custom Docker image.
...
- **Has:** DejaVuSans font
- **Missing:** Anton font (required for brand typography)
- **Missing:** Space Mono font (required for captions/data)
- **Missing:** JetBrains Mono font (required for telemetry)
...
COPY fonts/SpaceMono-Regular.ttf /usr/share/fonts/truetype/spacemono/
COPY fonts/SpaceMono-Bold.ttf /usr/share/fonts/truetype/spacemono/
COPY fonts/JetBrainsMono-Regular.ttf /usr/share/fonts/truetype/jetbrains/
RUN fc-cache -f -v
```
