...
for i in $(seq 0 $((CLIP_COUNT - 1))); do
    ...
    # Step 2: Overlay word-pop captions via Remotion CaptionTrack
    # Requires SRT/caption data alongside timestamps JSON.
    # Caption file expected at: same dir as timestamps, named {title}.srt
    CAPTION_FILE="$(dirname "$TIMESTAMPS_JSON")/${TITLE}.srt"
    CAPTIONED_FILE="$OUTPUT_DIR/${TITLE}-captioned.mp4"

    if [[ -f "$CAPTION_FILE" ]] && command -v npx &> /dev/null; then
        echo "  Adding captions from: $CAPTION_FILE"
        # Render CaptionTrack overlay with Remotion, then composite
        npx remotion render BowTieCaptionTrack \
            --props="{\"srtPath\":\"$CAPTION_FILE\"}" \
            --codec=prores --prores-profile=4444 \
            --pixel-format=yuva444p \
            "$OUTPUT_DIR/${TITLE}-captions-overlay.mov" 2>/dev/null

        if [[ -f "$OUTPUT_DIR/${TITLE}-captions-overlay.mov" ]]; then
            # Composite caption overlay on top of extracted clip
            ffmpeg -y -i "$CLIP_FILE" -i "$OUTPUT_DIR/${TITLE}-captions-overlay.mov" \
                -filter_complex "[0:v][1:v]overlay=0:0:shortest=1[v]" \
                -map "[v]" -map "0:a?" \
                -c:v libx264 -crf 18 -preset medium \
                -c:a aac -b:a 128k \
                "$CAPTIONED_FILE" 2>/dev/null
            rm -f "$OUTPUT_DIR/${TITLE}-captions-overlay.mov"
            # Use captioned file for concat
            CLIP_FILE="$CAPTIONED_FILE"
            echo "  Captioned: $CAPTIONED_FILE"
        else
            echo "  WARNING: Caption render failed, using uncaptioned clip"
        fi
    else
        echo "  No caption file found at $CAPTION_FILE — skipping captions"
    fi

    # Step 3: Concatenate intro + clip + outro
    ...
    # Cleanup temp files
    rm -f "$OUTPUT_DIR/${TITLE}-raw.mp4" "$OUTPUT_DIR/${TITLE}-captioned.mp4" "$CONCAT_LIST"
