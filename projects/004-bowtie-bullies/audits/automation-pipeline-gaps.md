...
| WF05-02 | **Font files not verified for n8n server** -- Blueprint specifies Bangers, Montserrat, Anton. Haven's WF-006 already discovered font path misalignment (Inter-Bold.ttf vs DejaVuSans-Bold.ttf). These 3 fonts are almost certainly NOT installed on the Hostinger server. | CRITICAL | All text overlays will fail. Must install fonts on server or use pre-rendered overlay images. |
...
10. **Install fonts on n8n server** -- Bangers, Montserrat, Anton (WF05-02)
