---
name: vibe-security
description: Security audit for AI-generated code — exposed keys, broken RLS, auth bypass, payment manipulation.
triggers:
  - "security audit"
  - "security review"
  - "vibe security"
...
license: MIT
metadata:
  author: Chris Raroque (Aloa)
  version: "1.0"
  source: https://github.com/raroque/vibe-security-skill
---

# Vibe Security — AI Code Security Auditor
...
- **Framework**: Next.js 13.4.19 (Pages Router, `output: 'export'` — static HTML)
- **Auth**: Supabase JS client with direct REST fetch + localStorage tokens
- **Database**: Supabase PostgreSQL (community platform + LFR instance)
- **Payments**: Stripe (Stan Store integration, course access gating)
- **AI APIs**: Gemini API (image generation), LLM pipelines via n8n
- **Hosting**: hosting.com (LiteSpeed) + Cloudflare CDN
- **Key concern**: Static export means NO server-side rendering — all "server" logic runs in Supabase Edge Functions or n8n webhooks
...
2. **Database Access Control** — Check Supabase RLS policies. This is the #1 source of critical vulnerabilities in vibe-coded apps. See `references/database-security.md`.

3. **Authentication & Authorization** — Validate JWT handling, auth flow protection, and session management. See `references/authentication.md`.

4. **Rate Limiting & Abuse Prevention** — Ensure auth endpoints, AI calls, and expensive operations have rate limits. See `references/rate-limiting.md`.
...
6. **Mobile Security** — If applicable. Verify secure token storage, API key protection via backend proxy, and deep link validation. See `references/mobile.md`.
...
- If the codebase doesn't use a particular technology (e.g., no Firebase), skip that section entirely.
